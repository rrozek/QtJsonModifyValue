...
class Json {
private:
    static bool debugMode = false; // show error messages?
    static bool strictMode = false; // if strict mode is active and any part of path does not exist, the function will not create it and will return false
...
public:
    ...
    static void modifyJsonValue(QJsonValue &destValue, const QString &path, const QJsonValue &newValue = QJsonValue(QJsonValue::Undefined))
    {
    ...
    static void modifyJsonValue(QJsonDocument& doc, const QString& path, const QJsonValue& newValue = QJsonValue(QJsonValue::Undefined))
    {
        QJsonValue val;
        if (doc.isArray())
            val = doc.array();
